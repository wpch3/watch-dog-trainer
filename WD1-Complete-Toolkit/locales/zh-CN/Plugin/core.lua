...
return function(env, catalog, receipt, mode)
    ...
    function c:arm()
        ...
        self:message("已激活本次单机测试；所有持续开关仍为关闭。")
        return true
    end
    ...
    function c:prepareItems(ids, quantity)
        ...
        self:message("待确认："..#jobs.." 项，每项 "..quantity.."。点击 [CONFIRM] 才会给予；[CANCEL] 取消。")
        return true
    end
    ...
    function c:prepareCash(amount)
        ...
        self:log("PREPARED_CASH","amount="..amount); self:message("待确认：现金 +"..amount.."。点击 [CONFIRM] 执行。")
        return true
    end
    function c:confirm()
    ...
    function c:spawn(index)
        ...
        self:message("已请求生成车辆；这不是叫车解锁，也不增加线上比赛次数。")
        return true
    end
    function c:tick()
        ...
        if #self.queue>0 and self.frames%4==0 then
            ...
            if #self.queue==0 then self:log("BATCH_SUBMITTED_NOT_VERIFIED"); self:message("本批调用已提交，未回读。请核对物品/点数，再正常存档、重启验证。") end
        end
    end
    function c:probe()
        ...
        self:message("只读能力报告已写入宿主日志，仅列函数名，不扫描内存或读取账号。")
    end
    return c
end
