...
for _,mutate in ipairs({function(r)r.allow_persistent=false end,function(r)r.format=2 end,function(r)r.mode='dlc_solo' end,function(r)r.backup_id='' end,function(r)r.product='other' end,function(r)r.expires_unix=1000000 end,function(r)r.created_unix=11000 end,function(r)r.expires_unix=9999 end,function(r)r.created_unix=10010;r.expires_unix=10000 end}) do
...
do local c,e,s,count,tick=fixture();e.os=nil;c:arm();check(not c:prepareItems({point},1),'no clock -> no persistent writes');check(c:toggle('god',true),'temporary god independent of receipt');end
do local c,e,s,count,tick=fixture();c:arm();c:prepareItems({point},1);c:confirm();s.now=17000;tick(4);equal(count('AddItem'),0,'expiry between confirmation and execution guarded');check(not c.armed,'expired queue disarms');end
...
do local c,e,s,count,tick=fixture();c:arm();local ids={'item_2207476061','item_4099987261','item_770883049','item_309961700'};c:prepareItems(ids,1);c:confirm();tick(4);c:pause();tick(30);equal(count('AddItem'),1,'pause cancels partially processed batch');c:resume();c:arm();tick(8);equal(count('AddItem'),1,'cancelled queue never silently restarts');end
...
do local c,e,s,count,tick=fixture();c:arm();e.AddItem=function()error('fixture')end;c:prepareItems({point},1);c:confirm();tick(4);check(not c.armed,'native exception halts queue');equal(c.submitted,0,'failed call not counted submitted');end
do local c,e,s,count,tick=fixture();c:arm();e.AddItem=function()return false end;c:prepareItems({point},1);c:confirm();tick(4);equal(c.submitted,0,'explicit false native result rejected');end
