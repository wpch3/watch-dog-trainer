...
"1718896871"}
    ...
    local function expShorts(full)
      local rest = full:gsub("^CarHackingRewards%.Generic%.", "")
      local cls, model = rest:match("^([%w_]+)%.([%w_]+)$")
      local underscored = rest:gsub("%.", "_")
      return rest, model or rest, underscored
    end
    function c:expUnlockOne(i)
      local full = EXP_NAMES[i]
      if not full then self:log("UNLOCKEXP", "bad index "..tostring(i)); return end
      local clsmodel, model, underscored = expShorts(full)
      local p = self:player()
      local has = env.UnlockAndBuyCarOnDemand ~= nil
      local tries = {
        {"0_full", function() return env.UnlockAndBuyCarOnDemand(0, full) end},
        {"0_full_true", function() return env.UnlockAndBuyCarOnDemand(0, full, true) end},
        {"0_clsmodel", function() return env.UnlockAndBuyCarOnDemand(0, clsmodel) end},
        {"0_model", function() return env.UnlockAndBuyCarOnDemand(0, model) end},
        {"0_underscored", function() return env.UnlockAndBuyCarOnDemand(0, underscored) end},
        {"p_full", function() return env.UnlockAndBuyCarOnDemand(p, full) end},
      }
      local any = false
      for _, t in ipairs(tries) do
        local ok, res = pcall(t[2])
        if ok then any = true end
        self:log("UNLOCKEXP", full.." shape="..t[1].." api="..tostring(has).." ok="..tostring(ok).." ret="..tostring(res))
      end
      self:log("UNLOCKEXP_DONE", full.." any_ok="..tostring(any))
      self:message("Unlock attempt logged: "..model..". Save + quit to desktop + relaunch, then check Car On Demand.")
    end
    function c:expSpawnOne(i)
      local full = EXP_NAMES[i]
      if not full then self:log("SPAWNEXP", "bad index "..tostring(i)); return end
      local clsmodel, model, underscored = expShorts(full)
      local has = env.SpawnAIVehicleAndPassengerOnRoad ~= nil
      local tries = {
        {"model", function() return env.SpawnAIVehicleAndPassengerOnRoad(model) end},
        {"clsmodel", function() return env.SpawnAIVehicleAndPassengerOnRoad(clsmodel) end},
        {"underscored", function() return env.SpawnAIVehicleAndPassengerOnRoad(underscored) end},
        {"full", function() return env.SpawnAIVehicleAndPassengerOnRoad(full) end},
        {"model_0", function() return env.SpawnAIVehicleAndPassengerOnRoad(model, 0) end},
        {"model_true", function() return env.SpawnAIVehicleAndPassengerOnRoad(model, true) end},
        {"0_model", function() return env.SpawnAIVehicleAndPassengerOnRoad(0, model) end},
      }
      local any = false
      for _, t in ipairs(tries) do
        local ok, res = pcall(t[2])
        if ok then any = true end
        self:log("SPAWNEXP", full.." shape="..t[1].." api="..tostring(has).." ok="..tostring(ok).." ret="..tostring(res))
      end
      self:log("SPAWNEXP_DONE", full.." any_ok="..tostring(any))
      self:message("Spawn attempt logged: "..model..". Look around you in the world; if a car appeared, drive it once.")
    end
    function c:expUnlockAll()
      local okc = 0
      for i = 1, #EXP_NAMES do
        local full = EXP_NAMES[i]
        local ok = pcall(function() return env.UnlockAndBuyCarOnDemand(0, full) end)
        if ok then okc = okc + 1 end
        self:log("UNLOCKEXP_ALL", full.." ok="..tostring(ok))
      end
      self:log("UNLOCKEXP_ALL_DONE", "total="..#EXP_NAMES.." ok="..okc)
      self:message("Bulk unlock pass logged: "..okc.."/"..#EXP_NAMES.." returned clean. Save, quit, relaunch, check Car On Demand.")
    end
    return c
end
