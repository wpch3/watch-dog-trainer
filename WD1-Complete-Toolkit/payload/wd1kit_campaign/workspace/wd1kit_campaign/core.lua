...
return function(env, catalog, receipt, mode)
    ...
    function c:prepareItems(ids, quantity)
        ...
        self:message("Prepared: "..#jobs.." entries, each "..quantity..". Press [CONFIRM] to grant, or [CANCEL] to cancel.")
        return true
    end
    ...
    function c:prepareCash(amount)
        ...
        self:log("PREPARED_CASH","amount="..amount); self:message("Prepared: cash +"..amount..". Press [CONFIRM] to apply.")
        return true
    end
    function c:confirm()
    ...
    function c:tick()
        ...
        if #self.queue>0 and self.frames%4==0 then
            ...
            if #self.queue==0 then self:log("BATCH_SUBMITTED_NOT_VERIFIED"); self:message("Batch submitted without readback. Check items or points, save normally, then restart to verify.") end
        end
    end
    function c:probe()
