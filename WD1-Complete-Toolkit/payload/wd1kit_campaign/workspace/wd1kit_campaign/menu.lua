...
return function(c, catalog, mode)
    ...
    register("int","exp_index","Garage experiment index (1-27, see 09 page)",1,1,27)
    ...
    garage:Layout(function(root)
        local ui=WD1KIT_UI2;ui:header(root);ui:confirmButtons(root)
        ui:text(root,"The 45 hidden Car-On-Demand records ARE CarHackingRewards entries (car delivery / hacking reward cars). This page tries the OFFICIAL names: unlock API + world spawn. Zero archive edits - NGM stays untouched.")
        ...
        ui:button(root,"[EXP] Unlock ALL 45 official names in one pass",function() WD1KIT_UI2.core:expUnlockAll() end)
    ...
    c:log("PLUGIN_LOADED","version=0.4.21-exp1-ui4 mode="..mode.." game_runtime_verified=false")
