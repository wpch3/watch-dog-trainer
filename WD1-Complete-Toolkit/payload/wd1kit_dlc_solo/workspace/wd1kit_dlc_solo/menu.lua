...
return function(c, catalog, mode)
    ...
    register("int","exp_index","Garage experiment index (1-73, see 09 page)",1,1,73)
    ...
    local garage=SH_Menu_RegisterCategory("09 / Garage Experiment (CarHackingRewards)",menu)
    garage:Layout(function(root)
        local ui=WD1KIT_UI2;ui:header(root);ui:confirmButtons(root)
        ui:text(root,"All 73 CarHackingRewards records (65 real vehicles: hidden cars, progression rewards, Stealth pre-order car, Quinn Limo, CrispinCar, emergency fleet, bikes) are ALREADY in your NGM database. This page tries the OFFICIAL names: unlock API + world spawn. Zero archive edits - NGM stays untouched.")
        ui:text(root,"Results are logged to WD1KIT_log.txt as UNLOCKEXP / SPAWNEXP lines. After an unlock attempt: SAVE, quit to desktop, relaunch, then check Car On Demand.")
        ui:widget(root,"exp_index")
        ui:button(root,"[EXP] Show name for index",function() local u=WD1KIT_UI2;u:message("EXP "..tostring(u:integer("exp_index")).." = "..u.core:expName(u:integer("exp_index"))) end)
        ui:button(root,"[EXP] Unlock attempt for index (official name shapes)",function() local u=WD1KIT_UI2;u.core:expUnlockOne(u:integer("exp_index")) end)
        ui:button(root,"[EXP] Spawn attempt for index (if a car appears, drive it once to keep it)",function() local u=WD1KIT_UI2;u.core:expSpawnOne(u:integer("exp_index")) end)
        ui:button(root,"[EXP] Unlock ALL 73 official names in one pass",function() WD1KIT_UI2.core:expUnlockAll() end)
        ui:text(root,"Offline only. Save-local unlock attempts, same class as the existing unlock features. Side effects are disclosed here and logged.")
    end)
    c:log("PLUGIN_LOADED","version=0.4.21-exp2-ui4 mode="..mode.." game_runtime_verified=false")
